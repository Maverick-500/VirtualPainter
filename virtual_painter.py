...
def main():
    global brush_color, brush_thickness, background_color, color_index
    ...
    while True:
        ...
        if result.multi_hand_landmarks:
            ...
            finger_pos = get_index_finger_tip(hand_landmarks, (h, w))
            x, y = finger_pos
        ...
        for i, text in enumerate(instructions):
            cv2.putText(combined, text, (10, 30 + i*30), cv2.FONT_HERSHEY_SIMPLEX, 0.7, (255,255,255), 2)
        ...
        if key == ord('c'):
        ...
        elif key == ord('+'): brush_thickness +=1
        elif key == ord('-'): brush_thickness = max(1, brush_thickness-1)
        elif key == ord('z'): brush_thickness = default_brush_thickness
        elif key == ord('e'):
            if not eraser_mode:
                prev_brush_color = brush_color
                prev_brush_thickness = brush_thickness
            ...
            shape_mode = 'line'; shape_start = None; bucket_mode=False; eraser_mode=False
        elif key == ord('t'):
            shape_mode = 'rectangle'; shape_start=None; bucket_mode=False; eraser_mode=False
        elif key == ord('o'):
            shape_mode = 'circle'; shape_start=None; bucket_mode=False; eraser_mode=False
        elif key == ord('k'):
            background_color=(0,0,0); canvas[:]=background_color
        elif key == ord('w'):
            background_color=(255,255,255); canvas[:]=background_color
        elif key == ord('m'):
            background_color=(50,50,50); canvas[:]=background_color
        elif key == 27:  # ESC
            break
        elif key == ord(' '):
            if shape_mode and shape_start and drawing_mode and finger_pos:
                ...
                x0,y0 = shape_start
                x1,y1 = finger_pos
                if shape_mode=='line': cv2.line(canvas,(x0,y0),(x1,y1),brush_color,brush_thickness)
                elif shape_mode=='rectangle': cv2.rectangle(canvas,(x0,y0),(x1,y1),brush_color,brush_thickness)
                elif shape_mode=='circle':
